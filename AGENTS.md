...
## Programação de Pagamento da proposta/matrícula

- **Conceito:** consultor define na proposta (Create/Edit, card "Gerenciamento de Parcelamento" → seção "Programação de Pagamento"): `parcela_selecionada` (N×), `primeira_parcela_valor` + `primeira_parcela_data` (entrada), `dia_pagamento` (1–31). Regra: total fixo = N× valor da linha; 1ª vence em `primeira_parcela_data` com valor próprio opcional e as demais (2..N) são recalculadas p/ preservar o total (acerto de centavos na última; 1ª > total = inválido; N=1 → parcela única = 1ª); da 2ª em diante todo `dia_pagamento` (trava fim-de-mês); sem data, 1ª = próximo `dia_pagamento` ≥ hoje.
- **Persistência:** `orc.parcelamento` (+ linhas) em `store/update`; no aceite (`publicApprove`) congela em `config.financiamento_aprovado` = inputs + `programacao[{n, vencimento Y-m-d, valor}]` + `approved_at` (imutável; só `revokeApproval` reabre).
- **Patterns:** Strategy (`DueDateStrategyInterface` + `FirstDueDateThenFixedDayStrategy`), Builder (`PaymentScheduleBuilder`), Facade (`PaymentScheduleService::build/forMatricula/toHtmlList`) em `backend/app/Services/PaymentSchedule/` — fonte única p/ tela, contrato e (futuro) Asaas. Frontend espelha em `lib/paymentSchedule.ts` + `components/school/PaymentScheduleSection.tsx` (Create + Edit) e `ProposalViewContent.tsx` (resumo usa `buildPaymentSchedule` p/ exibir demais recalculadas).
- **Contrato:** `{tabela_parcelas}` (alias `{cronograma_parcelas}`) + escalares `{dia_vencimento_parcelas, dia_pagamento, data_primeira_parcela, primeira_parcela_data, primeira_parcela_valor, qtd_parcelas, total_parcelas, valor_parcela, desconto_pontualidade, parcela_com_desconto, texto_desconto, valor_total}`, injetados no `$dm` em `contratos_periodos()` (aluno e responsável) via `injectTabelaParcelas()`. Linha ativa resolvida via `PaymentScheduleService::resolveDiscountDetails()`. A tabela `{tabela_parcelas}` renderiza os valores nominais e anexa automaticamente no rodapé a nota de desconto por pontualidade (condicionado ao pagamento em dia). Inserir `{tabela_parcelas}` ou tags nos contratos (`/admin/school/contracts`). Sem plano → string vazia.
- **Catálogo de shortcodes:** `frontend/src/lib/contractShortcodes.ts` (fonte única p/ chips, "Copiar Lista Base" e autocomplete). Editor `RichTextEditor` com opt-in `enableShortcodeHints` + `shortcodes`: digitar `{` filtra, `Shift+Espaço` abre a paleta, `Enter`/`Tab` insere, `Escape` fecha. Form `ContractForm` possui barra de atalhos rápidos de vendas, filtro por categoria (⭐ Vendas / Parcelamento, Aluno, Responsável, Curso, Datas) e busca por tag.
- **Validação backend:** `rules()` p/ `orc.parcelamento.*` + `validateParcelamentoSnapshot()` (selecionada ∈ linhas; programação construtível) chamada em `store/update` (422).
- **Teste:** `tests/Unit/PaymentScheduleServiceTest.php` (19 testes, sem DB).

## Integração Asaas (padrão Help Desk)

- **Card:** `frontend/src/components/settings/AsaasSettingsCard.tsx` (porte fiel do Help Desk: header gradiente, API Key c/ olho, Webhook Token, Ambiente, Cobrança Padrão, URL do webhook c/ copiar, status Conectado/Configurado/Não configurado + Testar Conexão), renderizado no topo de `pages/settings/Integrations.tsx`. Tipos em `types/asaas.ts`, service em `services/asaasService.ts` (`GET /asaas/status`, `POST /asaas/test-connection`).
- **Backend:** `Services/Asaas/AsaasService.php` (lookup `post_name='integracao-asaas'` ou `like '%asaas%'`, fallbacks `pass→access_token→api_key→token`; `isConnected` via `GET /finance/balance`; sem `organization_id` — CRM não tem a coluna) + `api/AsaasController.php` (`status`, `testConnection`, gate `permission_id ∈ {1,2}`) + rotas em `tenant.php` (grupo auth).
- **Credencial:** `ApiCredentialController::index` faz auto-bootstrap `integracao-asaas` (url sandbox, `billing_type=BOLETO`); validação `store`/`update` aceita `access_token/environment/billing_type/webhook_token`; cripto estendida p/ `access_token` + `webhook_token` (padrão do `pass`, sem espelho plaintext).
- **Teste:** `tests/Unit/AsaasServiceTest.php` (4 testes, `Http::fake`, sem DB; `uses(Tests\TestCase::class)` p/ facades).
- **Fase 2 — cobrança no `signed`:** `AsaasService::findOrCreateCustomer` (por cpfCnpj) + `createBilling` (entrada → avulsa `value`; restante → 1x avulsa ou `installmentCount+totalValue`; `externalReference=matricula:{id}[:parcelas]`) + `GenerateAsaasBillingJob` (idempotente via meta `asaas_billing`; cria conta `receivable source=asaas_billing` p/ conciliação; `tries=3`) disparado em `ZapsingController::webhook` no `signed` **só do envelope do aluno**. Teste estendido (8 testes Asaas no total).
- **Fase 3 — webhook `PAYMENT_*` → ganho:** `case 'asaas'` em `WebhookController` (usa rotas genéricas `ANY /webhook/{endp1}`; valida header `asaas-access-token` vs credencial; idempotência rápida por `event.id`; resolve matrícula por `externalReference`, detalhe `GET /payments/{id}` ou vínculo `installment_id` na meta; enfileira e responde 2xx). `ProcessAsaasPaymentJob`: baixa (`PAYMENT_RECEIVED/CONFIRMED` → payment `source=asaas_payment`, recalc pending/partial/paid), ganho aos 100% (espelha `updateStatusRapid 'g'`: `status`, situação `mat`, metas `data_ganho/valor_*`, `EventLog`), vencido (`overdue`), estorno/exclusão (remove baixa + recálcula; parcial → flag `asaas_review_*` p/ revisão manual). `AsaasPaymentMapper` puro + `AsaasService::getPayment`. Testes `AsaasPaymentMapperTest` (3, sem DB). Suite: 33 falhas pré-existentes inalteradas.
- **Mora/desconto + edição/exclusão:** credencial `integracao-asaas` += `fine_value/fine_type/interest_value` (vazio = padrão da conta; card Asaas c/ seção "Multa e Juros de mora"); `PaymentScheduleService::build` inclui `desconto_pontualidade` (linha selecionada → congela no snapshot); `createBilling` anexa `discount {FIXED, dia 0}` + `fine/interest` (nunca envia vazio); `AsaasService::updatePayment (PUT) / deletePayment (DELETE) / cancelInstallment (DELETE /installments/{id}/payments)`; `AsaasController`: `GET billing/{matricula}`, `PUT/DELETE billing/payments/{id}` (paga → 409), `DELETE billing/installments/{id}`; frontend `AsaasBillingSection` em `ProposalsView` (status vivo, editar, excluir avulsa/plano; paga sem botão excluir).
- **Próximas fases:** (concluído o ciclo proposto).

## Menu lateral (AppSidebar) — fonte e fallback

- **Fonte oficial:** `backend/database/seeders/data/menu_crm.json` (8 grupos, 42 subitens) → `MenuSeeder` popula a tabela `menus` → API (`MenuService`, `items` como filhos) → `AppSidebar` monta via `buildMenuFromDTO`.
- **Fallback:** `frontend/src/lib/menu.ts` → `defaultMenu` **espelha o `menu_crm.json` 1:1** (mesmos títulos, URLs relativas, ícones e submenus). Ao alterar o JSON, atualizar o `defaultMenu` junto.
- URLs relativas (ex.: `/school/enroll`) ganham prefixo `/admin` via `resolveUrl()` no `AppSidebar`.
- Ícones do JSON mapeados no `iconMap`: `Shop` não existe no lucide-react instalado → alias para `Store`; `Cog` e `LineChart` importados diretamente.
