...
- **Contrato:** `{tabela_parcelas}` (alias `{cronograma_parcelas}`) + escalares `{dia_vencimento_parcelas, dia_pagamento, data_primeira_parcela, primeira_parcela_data, primeira_parcela_valor, qtd_parcelas, total_parcelas, valor_parcela, desconto_pontualidade, parcela_com_desconto, texto_desconto, valor_total}`, injetados no `$dm` em `contratos_periodos()` (aluno e responsável) via `injectTabelaParcelas()`. Linha ativa resolvida via `PaymentScheduleService::resolveDiscountDetails()`. Inserir `{tabela_parcelas}` ou tags nos contratos (`/admin/school/contracts`). Sem plano → string vazia.
...
- **Teste:** `tests/Unit/PaymentScheduleServiceTest.php` (7 testes, sem DB).
