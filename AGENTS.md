...
- **Conceito:** consultor define na proposta (Create/Edit, card "Gerenciamento de Parcelamento" → seção "Programação de Pagamento"): `parcela_selecionada` (N×), `primeira_parcela_valor` + `primeira_parcela_data` (entrada), `dia_pagamento` (1–31). Regra: 1ª vence em `primeira_parcela_data`; da 2ª em diante todo `dia_pagamento` (trava fim-de-mês); sem data, 1ª = próximo `dia_pagamento` ≥ hoje.
...
- **Teste:** `tests/Unit/PaymentScheduleServiceTest.php` (12 testes, sem DB).
