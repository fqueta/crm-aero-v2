...
export default function IntegrationsEdit() {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => {
      toast.success('Configurações salvas no Banco Central');
  ...
  const handleTestConnection = () => {
    ...
    setTimeout(() => {
      ...
      if (url.trim() !== '') {
        setTestSuccess(true);
        toast.success(`Conexão com ${name} estabelecida com sucesso!`);
      } else {
    ...
    <div className="w-full max-w-7xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm text-slate-500 flex items-center gap-2">
          ...
          Configure a conta da plataforma para este serviço.
        </p>
      </div>
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded border border-white/20 text-slate-300 flex items-center gap-1">
                <Check className="w-3 h-3" /> Gravado no Banco Local
              </span>
            </div>
            <p className="text-slate-400 text-sm mt-1 max-w-xl">
              Esta configuração é exclusiva da sua plataforma. Ela tem prioridade sobre as variáveis de ambiente e é armazenada com segurança no banco de dados local (tenant).
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              <div className="space-y-6">
                ...
                <div className="pt-4">
                  ...
                  >
                    ...
                    {testing ? 'Testando conexão...' : testSuccess === true ? 'Conexão OK' : testSuccess === false ? 'Falha na conexão' : `Testar Conexão com ${name.split(' ')[0]}`}
                  </Button>
                </div>
              </div>
            </CardContent>
          ...
          <Button 
            ...
            disabled={!name || !url || saveMut.isLoading || isLoading}
          >
              ...
              <div className="space-y-6">
                ...
                <div className="pt-4 border-t border-slate-100">
                  <p className="text-sm text-emerald-600 font-semibold flex items-center gap-2">
                    <Check className="w-4 h-4" /> Chave ativa via banco local
                  </p>
                </div>
              </div>
            </CardContent>
